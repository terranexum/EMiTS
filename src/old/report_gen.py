...
def generate_report_content(file_path, pdf_path, doc, can):
    ...
    def page5(canvas, doc):
        ...
        if payback_flag == False and no_elec_flag == False:
            ...
            s = "Since the " + asap_area + " does not have electricity customers according to the records from the public utility, numbers cannot be reported right now because we do not yet have a system to help with natural gas emissions."
            wrap(canvas, s, "Helvetica", 16, 75, 0.75 * inch, 9.55 * inch)
    ...
    content.append(Spacer(10, 0.2 * inch))
    content.append(Paragraph(asap_area, styles['Heading1']))
    content.append(Paragraph(asap_date, styles['Heading2']))
    ...
    content.append(Spacer(3, 0.2 * inch))
    ...
    for file in files:
        ...
        if file.endswith('.xls'):
            ...
            try:
                ...
                report = generate_report_content(file_path, pdf_path, doc, can)
